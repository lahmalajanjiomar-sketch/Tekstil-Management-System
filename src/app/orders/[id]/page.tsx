...
export default function OrderDetailPage({ params }: { params: { id: string } }) {
  ...
  const { orders, products, customers, updateOrderStatus } = auth

  const order = orders.find((o) => o.id === params.id)
  const customer = order ? customers.find(c => c.id === order.customerId) : null;

  if (!order || !customer) {
    return <AppLayout><PageHeader title="Order not found" /></AppLayout>
  }
  ...
  return (
    <AppLayout>
      <PageHeader
        title={`${t('sidebar_orders')} #${order.id.slice(-6)}`}
        actions={
          ...
          <Card>
            ...
            <CardContent className="grid sm:grid-cols-3 gap-4">
              <div className="flex flex-col">
                ...
                <span className="font-medium">{order.createdAt ? format(parseISO(order.createdAt), "dd/MM/yyyy HH:mm") : '...'}</span>
              </div>
              <div className="flex flex-col">
